...
class ShowPost extends Component {
    ...
    componentDidMount(){
        ...
        let profileRef = storage.ref("Profile_Cover/profile"+this.userId+"/");
        ...
        db.collection("users").doc(this.userId).get().then((doc)=>{
            this.setState({name:doc.data().name, country:doc.data().country, city:doc.data().city})
        })
        .catch(err=>{
        ...
        .then(docc=>{
            if(docc){
                ...
                .then(docc=>{
                    docc.forEach(res=>{
                        storage.ref("Profile_Cover/profile"+res).getDownloadURL()
                        .then(url=>{
                            temp.push(url)
    ...
    addLikeHandler=()=>{
        ...
        likesSet.add(initialize.auth().currentUser.uid)
